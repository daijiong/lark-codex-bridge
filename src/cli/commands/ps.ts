...
/**
 * Pretty-print the list of running lark-codex-bridge processes.
 *
...
export async function runStopCli(target: string | undefined): Promise<void> {
  if (!target) {
    console.error('用法: lark-codex-bridge stop <bot id 或序号>');
    process.exit(1);
  }
  const entry = resolveTarget(target);
  if (!entry) {
    console.error(`✗ 没找到匹配的 bot:${target}`);
    console.error('  用 `lark-codex-bridge ps` 看可选目标。');
    process.exit(1);
  }
